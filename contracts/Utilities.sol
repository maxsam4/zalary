...
library Utilities {
  ...
  function onlySalaryDapp(address sender, address zalaryFactory) external pure {
  ...
  function _validateDefaultProofs(
    ...
    require(_noteCoderToStruct(_proof2InputNotes.get(1)).noteHash == _salaryVariables.currentSalaryBalance, 'salary note in 2 is not correct');

  }


  function _validateSalaryProof(
  ...
  function _processSalaryWithdrawal(
    bytes calldata _proof2,
    bytes calldata _proof1OutputNotes,
    SalaryVariables storage _salaryVariables
  ) external returns (bytes32 newcurrentSalaryBalance) {


    (bytes memory _proof2Outputs) = ACE(_salaryVariables.aceAddress).validateProof(JOIN_SPLIT_PROOF, address(this),
                                                                                 _proof2);
    ...
    require(_noteCoderToStruct(_proof2InputNotes.get(0)).noteHash == _salaryVariables.currentSalaryBalance, 'salary note in 2 is not correct');
  ...
  function _processAdjustSalary(
    ...
    require(_noteCoderToStruct(_proofInputNotes.get(0)).noteHash == _salaryVariables.currentSalaryBalance, 'salary note does not match input note');
  ...
  function _processSalarySettlement(
    bytes calldata _proofData,
    SalaryVariables storage _salaryVariables
  ) external {
  ...
  function _processsalaryRepayment(
    bytes calldata _proof2,
    bytes calldata _proof1OutputNotes,
    SalaryVariables storage _salaryVariables
  ) external {
