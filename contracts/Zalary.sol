...
contract Zalary is ZkAssetMintable {
  ...
  function withdrawSalary(
    ...
    uint256 _salaryDurationToWithdraw
  ) public {
    (,bytes memory _proof1OutputNotes) = Utilities._validateSalaryProof(_proof1, _salaryDurationToWithdraw, salaryVariables);

    require(_salaryDurationToWithdraw.add(salaryVariables.lastSalaryPaymentDate) < block.timestamp, ' withdraw is greater than accrued salary');
